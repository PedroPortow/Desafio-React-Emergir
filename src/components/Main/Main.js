...
function Main({dataNotes, dataPlantation, dataFarm, dataRain}) {

  const notesFarmType = []
  const notesPlantationType = []
  
  dataNotes.results.filter((el) => {
    ...
    return '' // Só botei isso pro eslint parar de reclamar
  })
    ...
    <div id="main">
          <div id='mainWrapper'>
              ...
              <div className='card-row'>
                ...
                {dataPlantation.results.map((el, index) => (
                  <HeaderTalhao 
                    key={el.id} 
                    el={el}
